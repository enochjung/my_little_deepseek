...
use config::{CompositionExclusionFormat, Format, UnicodeFormat};
use core::MLTError;
...
impl Normalizer {
    pub fn new(
        ...
        let (file, parse) = unicode_format.read()?;
        let iter = parse(&file);

        for ucdline in iter {
            let ucdline = ucdline?;
            let codepoint = ucdline.codepoint;
            let combining_class = ucdline.combining_class;
            let decomposition = ucdline.decomposition;
        ...
        let (file, parse) = composition_exclusion_format.read()?;
        let iter = parse(&file);
    ...
    pub fn execute(&self, input: &str) -> Result<String, MLTError> {
        if input.is_ascii() {
        ...
        recomposed
            ...
            .try_fold(String::with_capacity(input.len()), |mut out, codepoint| {
                let Some(ch) = char::from_u32(codepoint) else {
                    return Err(MLTError::invalid_char(codepoint));
                };
                out.push(ch);
                Ok(out)
            })
    ...
    fn flush_recompose_segment(&self, out: &mut Vec<u32>, segment: &mut Vec<u32>) {
        ...
        for codepoint in segment.drain(..).skip(1) {
            ...
            if !blocked
                && let Some(composed) = self.composition_map.get(starter, codepoint)
                && !self.exclusion_map.contains(composed)
            {
                starter = composed;
                continue;
            }
