...
use tensor::{BF16, F32, Host, Tensor, TensorRef};

pub struct WordEmbeddingEngine<'a> {
    _model_data: &'a ModelData,
    table: Tensor<F32, Hsot>,
}

impl<'a> WordEmbeddingEngine<'a> {
    pub fn new(model_data: &'a ModelData) -> Result<Self, Error> {
        ...
        let table = Tensor::<F32, Host>::from(&tensor);
    ...
    pub fn word_embed<'b>(&'b self, token_id: u32) -> Result<TensorRef<'b, F32>, Error> {
        let nrow = self.table.layout().nrow;
        let ncol = self.table.layout().ncol;
        let row = token_id as usize;
        if row >= nrow {
            return Err(Error::out_of_bound(row, nrow));
        }

        self.table.as_ref(row..(row + 1), 0..ncol)
    }
}
...
fn parse_embed_tokens_text<'a>(_weight_text: &'a WeightText) -> Result<TensorRef<'a, BF16>, Error> {
    todo!()
