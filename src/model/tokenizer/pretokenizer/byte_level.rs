...
impl ByteLevelEngine {
    ...
    pub fn pretokenize(&self, input: &str, split_spans: &[(usize, usize)]) -> Vec<Vec<u8>> {
        split_spans
            .iter()
...
fn encode_token(token: &str) -> Vec<u8> {
    ...
    for &byte in token.as_bytes() {
        ...
        let mut buf = [0_u8; 4];
        result.extend_from_slice(mapped.encode_utf8(&mut buf).as_bytes());
    }
    result
}
...
mod tests {
    ...
    fn pretokenize_case1_hello() {
        let engine = ByteLevelEngine::new();
        assert_eq!(
            engine.pretokenize("Hello!", &[(0, 5), (5, 6)]),
            vec![tok("Hello"), tok("!")],
        );
    }
